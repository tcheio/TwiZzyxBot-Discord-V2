...
module.exports = async function annonceYouTube(bot,message) {
    msg = message.content;
    if (message.channelId == config.channel.retour){ //Channel #TwiZzyx/TwiZzyxPasSympa
        ...
        bot.channels.cache.get(config.channel.log).send("Une rediff de stream a été publié à "+classique.temps());
    }
